...
export class EmploiComponent {
 routeData: Subscription | undefined;
  EmploiListSubscription: Subscription | undefined;
  emplois: IEmploi[] = [];
   ...
   ngOnInit(): void { 
      ...
      ngOnDestroy(): void {
        if (this.routeData) {
          ...
          if (this.EmploiListSubscription) {
            this.EmploiListSubscription.unsubscribe();
          }
        }
      }
      ...
      loadAll(): void {
        ...
        this.emploiService.query(req).subscribe(result => {
          if (result && result.body) {
            this.totalRecords = Number(result.headers.get('X-Total-Count'));
            this.emplois = result.body || [];
          }
        });
      }
      ...
      openModalCreate(): void {
        this.dialogService.open(CreerModifierEmploiComponent,
          {
            header: 'Ajouter une Emploi',
            width: '60%',
          ...
          if(result) {
          this.emplois.push(result.body);
          this.isDialogOpInProgress = false;
          this.showMessage({ severity: 'success', summary: 'Emploi creer avec succès' });
          }
        });
      }

      /** Permet d'afficher un modal pour la modification */
      openModalEdit(Emploi: IEmploi): void {
        this.dialogService.open(CreerModifierEmploiComponent,
          {
            header: 'Modifier un Emploi',
            width: '60%',
            contentStyle: { overflow: 'auto' },
            baseZIndex: 10000,
            maximizable: true,
      ...
      openModalDetail(emploi:IEmploi): void {
        this.dialogService.open(DetailsEmploiComponent,
          {
            header: 'Details de Emploi',
            width: '60%',
            contentStyle: { overflow: 'auto' },
            baseZIndex: 10000,
            maximizable: true,
      ...
      // Suppression
      onDelete(Emploi: IEmploi) {
        this.confirmationService.confirm({
          message: 'Etes-vous sur de vouloir supprimer ce Emploi?',
          accept: () => {
            this.delete(Emploi);
          }
        });
      ...
      delete(selection: any) {
        ...
        this.emploiService.delete(selection.id).subscribe(() => {
          this.emplois = this.emplois.filter(emploi => emploi.id !== selection.id);
          selection = null;
          this.isOpInProgress = false;
          this.totalRecords--;
          this.showMessage({
            severity: 'success',
            summary: 'Emploi supprimé avec succès',
          });
        }, (error) => {
