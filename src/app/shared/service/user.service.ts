...
export class UserService {
    ...
    create(user: IUser): Observable<EntityResponseType> {
        return this.http.post<IUser>(resourceUrl, user, {
            observe: 'response',
        });
    }

    validationCompte(
        validationCompte: IValidationCompte
    ): Observable<ValidationCompteResponseType> {
        return this.http.post<IValidationCompte>(
            resourceUrl + '/confirm-user',
            validationCompte,
            { observe: 'response' }
        );
    }
    ...
    updateuser(user: IUser): Observable<EntityResponseType> {
        ...
        return this.http.put<IUser>(utilisateurUrl, user, {
            observe: 'response',
        });
    }

    find(id: number): Observable<EntityResponseType> {
        return this.http.get<IUser>(`${resourceUrl}/${id}`, {
            observe: 'response',
        });
    }

    confirmeCompte(key: string): Observable<any> {
        return this.http.get(resourceUrl + '/confirm?token=' + key, {
            observe: 'response',
        });
    }
    ...
    query(req?: any): Observable<EntityArrayResponseType> {
        ...
        return this.http.get<IUser[]>(resourceUrl, {
            params: options,
            observe: 'response',
        });
    }
    ...
    ): Observable<PasswordResponseType> {
        return this.http.post<IChangePasswordDTO>(
            resourceUrl + '/reset-connect-password',
            request,
            { observe: 'response' }
        );
    }
    ...
    getUserByMatricule(matricule: string): Observable<IUser> {
        return this.http.get<IUser>(`${utilisateurUrl}/${matricule}`);
    }
    createCompte(user: IUser): Observable<EntityResponseType> {
        return this.http.post<IUser>(
            `${environment.authResource}/register`,
            user,
            {
                observe: 'response',
            }
    ...
    getUserByMatricule1(matricule: string): Observable<IUser> {
        return this.http.get<IUser>(`${utilisateurUrl}/user/${matricule}`);
    }
}
